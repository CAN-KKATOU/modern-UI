...
#include "PacketFixed.h"

namespace PktParse {
    PacketFixed::PacketFixed(const string &dataStr, const LengthSet &lengthSet)
            : Packet101(dataStr, lengthSet) {

    }

    shared_ptr<json> PacketFixed::toJson() {
    ...
    shared_ptr<json> PacketFixed::getPosition() {
        auto object = make_shared<json>();
    ...
    PacketFixed::PacketFixed(const LengthSet &lengthSet)
            : Packet101(lengthSet) {
    ...
    PacketFixed::PacketFixed(const LengthSet &lengthSet, int controlArea, size_t addressArea)
            : Packet101(lengthSet, controlArea, addressArea) {
        origin_data->push_back(16);
        origin_data->push_back(cArea);
        util::pushElems(origin_data, length.l_addr, aArea);
    ...
    PacketFixed::PacketFixed(const LengthSet &lengthSet, bool dir, bool prm, bool fcb, bool fcv, bool acd, bool dfc,
                             int fc, size_t addressArea)
            : Packet101(lengthSet, dir, prm, fcb, fcv, acd, dfc, fc, addressArea) {
        origin_data->push_back(16);
        origin_data->push_back(cArea);
        util::pushElems(origin_data, length.l_addr, aArea);
    ...
    PacketFixed::PacketFixed(const PacketFixed &rhs) : Packet101(rhs) {

    }

    PacketFixed &PacketFixed::operator=(const PacketFixed &rhs) {
        if (this == &rhs)
            return *this;
