...
namespace PktParse {
    Packet104::Packet104(const string &dataStr, const LengthSet &lengthSet)
    ...
    vector<string> Packet104::getAllTypes() {
        vector<string> vec;
    ...
    string Packet104::parse104Type(const string &dataStr, int typeLength) {
        auto data = make_shared<vector<int>>();
        *data = util::splitToInt(dataStr, " ");

        switch (select104Type(dataStr, data)) {
            case 1:
                return Packet104I::parseIType(dataStr, typeLength, data);
            case 2:
                return "S格式报文";
            case 3:
                return "U格式报文";
    ...
    int Packet104::select104Type(const string &dataStr, const shared_ptr<vector<int>> &dataVec) {
        auto data = make_shared<vector<int>>();
    ...
    int Packet104::parse104TypeInt(const string &dataStr, int typeLength) {
        auto data = make_shared<vector<int>>();
        *data = util::splitToInt(dataStr, " ");

        switch (select104Type(dataStr, data)) {
            case 1:
                return Packet104I::parseITypeInt(dataStr, typeLength, data);
            case 2:
                return -2;
            case 3:
                return -3;
